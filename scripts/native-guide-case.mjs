...
// The focused case runs only the create-guide flow, including the exact
// project path preview assertion. The frame checks need real OS window focus,
// which a Windows CI runner does not reliably grant.
...
try {
  ...
  if (focused) await checkGuide(context);
  else {
  ...
  const result = {ok: true, platform: process.platform, arch: process.arch, evidence: userData,
    checks: focused ? ['create-guide-project-path-preview-separator']
