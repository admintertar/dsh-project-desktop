...
资源可以使用自定义的项目内路径，也可以关联项目外的现有目录。`resources/` 是新建资源的默认位置，项目定义记录实际位置；空项目没有预置资源，因此不需要生成这个目录。全新检出的项目根下没有 `resources/`——根 Git 精确忽略每个子资源——因此第一次打开项目时，壳会按项目定义自动重建「Git 类型、声明了远端地址、目录缺失、没有本机绑定且没有既有克隆作业」的资源目录；打开不等待克隆，失败仍可在资源页手动重试。
